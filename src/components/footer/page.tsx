...
import { localizationFooter } from "@/local/footer";
import Image from "next/image";
import Link from "next/link";

export default function Footer({ lng }: { lng: "ru" | "en" | "uz" }) {

    return (
        <footer className='bg-footer Open_Sans '>

            <div className='container grid grid-cols-1 lg:grid-cols-2 gap-y-6 mx-auto py-6'>
                <h2 className='col-span-full xl:text-3xl  md:text-2xl'>{localizationFooter[lng]?.title}</h2>
                <article className='flex flex-col gap-6'>
                    <h3 className='text-xl'>{localizationFooter[lng]?.contacts?.title}</h3>
                    <div className=' text-sm flex flex-col gap-6 '>
                        <p>{localizationFooter[lng]?.contacts?.appeals}</p>
                        <p>{localizationFooter[lng]?.contacts?.department}</p>
                        <p>{localizationFooter[lng]?.contacts?.directorate}</p>
                        <p>{localizationFooter[lng]?.contacts?.email}</p>
                        <p>{localizationFooter[lng]?.contacts?.hours} </p>
                        <p>{localizationFooter[lng]?.contacts?.passage}</p>
                        <p>{localizationFooter[lng]?.contacts?.phone}</p>
                    </div>

                </article>
                <div className='lg:justify-self-end  flex flex-col gap-6'>
                    <h3 >{localizationFooter[lng]?.address}</h3>
